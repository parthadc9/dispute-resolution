...
func (t *TestChaincode) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	_, orgName := GetCreator(creatorBytes)
...
switch orgName {
    ...
    case "org3": // Regulatory Authority
        {
            switch fnc {
            case "ViewComplaint":
            case "GetComplaintStatus":
    ...
    case "org4": // Regulatory Authority
        {
            switch fnc {
            case "ViewComplaint":
            case "GetComplaintStatus":
	...
	switch fnc {
	case "GenerateComplaint":
		return t.GenerateComplaint(stub,args)
	case "ViewComplaint":
		return t.ViewComplaint(stub,args)
	case "ResolveComplaint":
		return t.ResolveComplaint(stub,args)
	case "GetComplaintStatus":
		return t.GetComplaintStatus(stub,args)
	}
